...
export class ScreenStore {
  ...
  update(model: Partial<IScreen>) {
    const update$ = this.httpClient
      .put<IScreen>(this.apiUrl + 'screens', model)
      .pipe(
        map((screens) => screens),
        catchError((error) => {
          this.notifyService.notify('error', error);
  ...
  delete(model: IScreen) {
    const delete$ = this.httpClient
      .delete<IScreen>(this.apiUrl + 'screens/' + model.id)
      .pipe(
        map((screen) => screen),
        catchError((error) => {
          this.notifyService.notify('error', error);
        ...
        tap((screen) => {
          const deleteFromSubject = produce(
  ...
  createScreenHeader(screenHeader: IScreenHeader) {
    const create$ = this.httpClient
      .post<IScreenHeader>(this.apiUrl + 'screenheaders', screenHeader)
      .pipe(
  ...
  updateScreenHeader(screenHeader: IScreenHeader) {
    const update$ = this.httpClient
      .put<IScreenHeader>(this.apiUrl + 'screenheaders', screenHeader)
      .pipe(
        map((screenheaders) => screenheaders),
        catchError((error) => {
          this.notifyService.notify('error', error);
        ...
        tap((screenheaders) => {
          ...
          this.notifyService.notify(
            'success',
            'Üst Başlık Bilgisi Güncellendi...'
          );
        })
      );
    this.loadingService.showLoaderUntilCompleted(update$).subscribe();
  ...
  createScreenFooter(screenHeader: IScreenFooter) {
    const create$ = this.httpClient
      .post<IScreenFooter>(this.apiUrl + 'screenfooters', screenHeader)
      .pipe(
  ...
  updateScreenFooter(screenHeader: IScreenFooter) {
    const create$ = this.httpClient
      .put<IScreenFooter>(this.apiUrl + 'screenfooters', screenHeader)
      .pipe(
        map((screenfooters) => screenfooters),
        catchError((error) => {
          this.notifyService.notify('error', error);
        ...
        tap((screenfooters) => {
          ...
          this.notifyService.notify(
            'success',
            'Alt Başlık Bilgisi Güncellendi...'
          );
        })
      );
    this.loadingService.showLoaderUntilCompleted(create$).subscribe();
  ...
  updateScreenHeaderPhoto(model: IScreenHeaderPhoto) {
    const update$ = this.httpClient
      .put<IScreenHeaderPhoto>(this.apiUrl + 'screenheaderphotos', model)
      .pipe(
        map((photo) => photo),
        catchError((error) => {
          this.notifyService.notify('error', error);
        ...
        tap((photo) => {
          const updatedSubject = produce(this.subject.getValue(), (draft) => {
            ...
            const photoIndex = draft[index].screenHeaderPhotos.findIndex(
              (x) => x.id === photo.id
            );
  ...
  setIsMainScreenHeaderPhoto(model: IScreenHeaderPhoto) {
    const update$ = this.httpClient
      .put<IScreenHeaderPhoto>(
        this.apiUrl + 'screenheaderphotos/setmain',
        model
      )
      .pipe(
        map((photo) => photo),
        ...
        tap((photo) => {
          const updatedSubject = produce(this.subject.getValue(), (draft) => {
            ...
            if (index != -1) {
              ...
              if (photoIndex != -1) {
                const isMain = draft[index].screenHeaderPhotos.find(
                  (x) =>
                    x.isMain == true &&
  ...
  deleteScreenHeaderPhoto(model: IScreenHeaderPhoto) {
    const delete$ = this.httpClient
      .delete<IScreenHeaderPhoto>(
        this.apiUrl + 'screenheaderphotos/' + model.id
      )
      .pipe(
        map((photo) => photo),
        catchError((error) => {
  ...
  getScreenById(screendId: number): Observable<IScreen> {
    return this.screens$.pipe(
      map((screens) => screens.find((x) => x.id === screendId))
    );
  }
}
